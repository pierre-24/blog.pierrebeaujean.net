...
When a potential difference is applied across the two plates of a capacitor, charges accumulate on them, creating an electric field between the plates. The charges on the two plates have opposite signs and therefore attract each other.
...
In fact, for the developments that follow, it might be more convenient to define this potential with respect to the PZC, as

$$\phi = \psi_0 - \psi_{\mathrm{PZC}},$$

so that $\sigma_M = 0$ at $\phi = 0$.
...
$$n_i(x)=n_0\exp\left(-\frac{z_i e\psi(x)}{k_B T}\right),$$

where $z_i$ is the valence of ion $i$, $e$ is the elementary charge, $k_B$ is the Boltzmann constant, and $T$ is the temperature. Here, $n_0$ is the bulk number concentration of each ionic species.

For a symmetric $z:z$ electrolyte, with $z_+=+z$ and $z_-=-z$, the two ionic concentrations are therefore

$$n_+(x)=n_0\exp\left(-\frac{ze\psi(x)}{k_B T}\right),\qquad n_-(x)=n_0\exp\left(\frac{ze\psi(x)}{k_B T}\right).$$
...
$$\rho(x)=ze\left[n_+(x)-n_-(x)\right]=-2zen_0\sinh\left(\frac{ze\psi(x)}{k_B T}\right).$$
...
$$2\frac{d\psi}{dx}\frac{d^2\psi}{dx^2}=\frac{4zen_0}{\varepsilon}\sinh\left(\frac{ze\psi}{k_B T}\right)\frac{d\psi}{dx}.$$

The left-hand side can be recognized as the derivative of $(d\psi/dx)^2$. Integrating both sides with respect to $x$ gives

$$\left(\frac{d\psi}{dx}\right)^2=\frac{4n_0k_BT}{\varepsilon}\cosh\left(\frac{ze\psi}{k_BT}\right)+C.$$
...
$$0=\frac{4n_0k_BT}{\varepsilon}+C\quad\Longrightarrow\quad C=-\frac{4n_0k_BT}{\varepsilon}.$$

Therefore,

$$\left(\frac{d\psi}{dx}\right)^2=\frac{4n_0k_BT}{\varepsilon}\left[\cosh\left(\frac{ze\psi}{k_BT}\right)-1\right].$$

Using the identity $\cosh(y)-1=2\sinh^2(y/2)$, this becomes

$$\left(\frac{d\psi}{dx}\right)^2=\frac{8n_0k_BT}{\varepsilon}\sinh^2\left(\frac{ze\psi}{2k_BT}\right).$$

Taking the negative square root, since the potential decreases with distance from a positively charged electrode, gives

$$\frac{d\psi}{dx}=-\sqrt{\frac{8n_0k_BT}{\varepsilon}}\sinh\left(\frac{ze\psi}{2k_BT}\right).$$

We can now connect the potential to the charge on the electrode. From Gauss's law, the surface charge density on the metal electrode, $\sigma_M$, is related to the electric field at the electrode surface:

$$\sigma_M=-\varepsilon\left.\frac{d\psi}{dx}\right|_{x=0}.$$

At the electrode surface, $x=0$ and $\psi=\psi_0$, so

$$\sigma_M=\sqrt{8\varepsilon n_0k_BT}\sinh\left(\frac{ze\psi_0}{2k_BT}\right).$$
...
$$C_{\mathrm{GC}}=\frac{d\sigma_M}{d\psi_0}=\sqrt{\frac{2z^2e^2\varepsilon n_0}{k_BT}}\cosh\left(\frac{ze\psi_0}{2k_BT}\right).$$

We can make this expression a little more intuitive by introducing the inverse **Debye screening length**,

$$\kappa=\sqrt{\frac{2z^2e^2n_0}{\varepsilon k_BT}}.$$

The Gouy–Chapman capacitance then takes the particularly simple form

$$C_{\mathrm{GC}}=\varepsilon\kappa\cosh\left(\frac{ze\psi_0}{2k_BT}\right).$$

The $\cosh$ term tells us something interesting: unlike the Helmholtz capacitance, the Gouy–Chapman capacitance is not constant. It increases with the magnitude of the electrode potential.
In fact, we will see below that it as a characteristic parabola-like shape.
...
Note that it is inverselly proportional to the concentration, $n_0$, which means that when $n_0\gg 0$, $\lambda_D\to 0$.
This is, obviously, unphysical.
There is, however, an interesting limiting case. When the surface potential is small, such that $ze\psi_0\ll k_BT$, we can use the approximation $\sinh(y)\approx y$. The electric-field equation then becomes
...
The potential drop across the compact layer is therefore

$$\psi_0-\psi_d=\frac{\sigma_Md}{\varepsilon}.$$
...
$$\sigma_M=\sqrt{8\varepsilon n_0k_BT}\sinh\left(\frac{ze\psi_d}{2k_BT}\right).$$

The Stern model is therefore obtained by combining these two equations.
...
$$\psi_0=\psi_d+\frac{\sqrt{8\varepsilon n_0k_BT}}{C_H}\sinh\left(\frac{ze\psi_d}{2k_BT}\right).$$

This equation generally cannot be inverted analytically to obtain $\psi_d$ as an explicit function of $\psi_0$. 
But that is not really a problem: it already gives us the full Stern model, and $\psi_d$ can be obtained numerically for any given electrode potential.
And we can go one step further and derive the differential capacitance of the complete EDL.
The total potential drop is the sum of the potential drops across the compact and diffuse layers:
...
$$\frac{d\psi_0}{d\sigma_M}=\frac{1}{C_H}+\frac{1}{C_{\mathrm{GC}}(\psi_d)}.$$

Taking the inverse gives the **Stern differential capacitance**:

$$\frac{1}{C_{\mathrm{S}}}=\frac{1}{C_H}+\frac{1}{C_{\mathrm{GC}}(\psi_d)}.$$
...
Using the Gouy–Chapman expression derived above for $C_H$ and $C_{GC}$, we finally obtain

$$\frac{1}{C_{\mathrm{S}}}=\frac{d}{\varepsilon}+\frac{1}{\varepsilon\kappa\cosh\left(ze\psi_d/(2k_BT)\right)}.$$

There is a small but important subtlety here: the Stern capacitance is not simply obtained by putting $\psi_0$ into the Gouy–Chapman expression. 
The Gouy–Chapman contribution depends on the potential **at the beginning of the diffuse layer**, $\psi_d$, rather than directly on the electrode potential $\psi_0$.
This distinction becomes increasingly important as the compact-layer potential drop becomes significant.
The Stern model indeed gaves us a useful compromise between the two limiting pictures.
...
In 1947, David C. Grahame refined the Stern model to account for the fact that some ions can lose part of their solvation shell and adsorb specifically onto the electrode surface, while others remain fully solvated.
This led to a distinction between two planes within the compact part of the EDL: the **Inner Helmholtz Plane (IHP)** and the **Outer Helmholtz Plane (OHP)**.
...
1. **Inner Helmholtz Plane (IHP), at $x=x_{\mathrm{IHP}}$**.
   The IHP is the locus of the centers of **specifically adsorbed ions**. These ions are typically partially or completely desolvated and can interact directly with the metal through short-range chemical and non-electrostatic interactions.
   Because these interactions are not purely electrostatic, specifically adsorbed ions can, in principle, adsorb even when they have the same sign of charge as the electrode.
...
2. **Outer Helmholtz Plane (OHP), at $x=x_{\mathrm{OHP}}$**.
   The OHP is the locus of the centers of **non-specifically adsorbed ions** at their distance of closest approach to the electrode.
   These ions retain their primary solvation shell and are assumed to interact with the electrode primarily through long-range electrostatic forces.
...
Between the metal and the IHP, there is no volume charge in the continuum description.
The potential therefore obeys the Laplace equation. If the dielectric permittivity in this region is $\varepsilon_{\mathrm{IHP}}$, the electric field is constant and determined by the metal surface charge:

$$\frac{d\psi}{dx}=-\frac{\sigma_M}{\varepsilon_{\mathrm{IHP}}}.$$

Integrating from the metal surface, where $\psi(0)=\psi_0$, to the IHP, where $\psi(x_{\mathrm{IHP}})=\psi_{\mathrm{IHP}}$, gives

$$\psi_{\mathrm{IHP}}-\psi_0=-\frac{\sigma_Mx_{\mathrm{IHP}}}{\varepsilon_{\mathrm{IHP}}}.$$

Thus, within this region,

$$\psi(x)=\psi_0-\frac{\sigma_M}{\varepsilon_{\mathrm{IHP}}}x,\qquad 0\leq x\leq x_{\mathrm{IHP}}.$$
...
$$\left.\frac{d\psi}{dx}\right| _ {x_{\mathrm{IHP}}^+}-\left.\frac{d\psi}{dx}\right| _ {x_{\mathrm{IHP}}^-}=-\frac{\sigma_{\mathrm{IHP}}}{\varepsilon_{\mathrm{OHP}}}.$$
...
$$\psi_{\mathrm{OHP}}-\psi_{\mathrm{IHP}}=-\frac{(\sigma_M+\sigma_{\mathrm{IHP}})(x_{\mathrm{OHP}}-x_{\mathrm{IHP}})}{\varepsilon_{\mathrm{OHP}}},$$
...
$$\psi_0-\psi_{\mathrm{OHP}}=\frac{\sigma_Mx_{\mathrm{IHP}}}{\varepsilon_{\mathrm{IHP}}}+\frac{(\sigma_M+\sigma_{\mathrm{IHP}})(x_{\mathrm{OHP}}-x_{\mathrm{IHP}})}{\varepsilon_{\mathrm{OHP}}}.$$

This equation is essentially the Grahame version of the compact-layer contribution.

For $x>x_{\mathrm{OHP}}$, the Gouy–Chapman description takes over. The diffuse layer starts at the OHP, so the relevant surface potential in the Poisson–Boltzmann solution is now $\psi_{\mathrm{OHP}}$, rather than the metal potential $\psi_0$.

For a symmetric $z:z$ electrolyte, the integrated Poisson–Boltzmann equation gives the diffuse-layer charge as

$$\sigma_d=-\sqrt{8\varepsilon_{\mathrm{bulk}}n_0k_BT}\sinh\left(\frac{ze\psi_{\mathrm{OHP}}}{2k_BT}\right).$$

Combining this with electroneutrality,

$$\sigma_d=-(\sigma_M+\sigma_{\mathrm{IHP}}),$$

links the potential at the OHP directly to the total charge stored in the compact part of the EDL.

The corresponding diffuse-layer capacitance is

$$C_{GC}(\psi_{\mathrm{OHP}})=\frac{d(-\sigma_d)}{d\psi_{\mathrm{OHP}}}=\varepsilon_{\mathrm{bulk}}\kappa\cosh\left(\frac{ze\psi_{\mathrm{OHP}}}{2k_BT}\right),$$

where $\kappa$ is the inverse Debye length.

At this point, we can regard the EDL as consisting of two main regions: a **compact layer**, extending from the metal surface to the OHP and containing the IHP, followed by the **diffuse layer** beyond the OHP. The total potential drop is consequently divided between these two regions.
...
The diffuse layer is described by the Gouy–Chapman capacitance, $C_{\mathrm{GC}}(\psi_{\mathrm{OHP}})$. Since the same interfacial charge must be balanced across the compact and diffuse layers, their differential capacitances combine in series. The total differential capacitance of the Grahame model, which we denote by $C_G$, is therefore

$$\frac{1}{C_G}=\frac{1}{C_{\mathrm{compact}}}+\frac{1}{C_{\mathrm{GC}}(\psi_{\mathrm{OHP}})}.$$

Unlike the simple Helmholtz capacitance, however, the compact-layer capacitance is not simply determined by the thickness and dielectric permittivity of the layer. The reason is that the charge associated with specifically adsorbed ions can itself change as the electrode charge changes.

From the potential drop derived above, we obtain
...
The important point is that the EDL is no longer just a question of electrostatics.

Once we distinguish the IHP from the OHP, we are implicitly acknowledging that **ion–surface interactions, solvation, and specific adsorption** can affect the structure of the interface.
